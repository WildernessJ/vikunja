...
END:VCALENDAR`,
			...
			wantProps: ParsedVTODOProperties{Title: true, Description: true, Repeat: true},
		},
		{
...
END:VCALENDAR`,
			...
			// Repeat stays false: an unsupported rule is dropped, so an update
			// must not wipe a stored recurrence either.
			wantProps: ParsedVTODOProperties{Title: true, Description: true},
		},
		{
...
END:VCALENDAR`,
			...
			wantProps: ParsedVTODOProperties{Title: true, Description: true, DueDate: true, Deadline: true},
		},
		{
			name: "with estimated duration",
			args: args{content: `BEGIN:VCALENDAR
VERSION:2.0
X-PUBLISHED-TTL:PT4H
X-WR-CALNAME:test
...
SUMMARY:Todo #1
DESCRIPTION:Lorem Ipsum
X-VIKUNJA-ESTIMATED-DURATION:5400
LAST-MODIFIED:00010101T000000
END:VTODO
END:VCALENDAR`,
			},
			...
			wantProps: ParsedVTODOProperties{Title: true, Description: true, EstimatedDuration: true},
		},
		{
			name: "with empty estimated duration (explicit clear, not omission)",
			args: args{content: `BEGIN:VCALENDAR
VERSION:2.0
X-PUBLISHED-TTL:PT4H
X-WR-CALNAME:test
...
END:VCALENDAR`,
			...
			wantProps: ParsedVTODOProperties{Title: true, EstimatedDuration: true},
		},
		{
			name: "with empty deadline (explicit clear, not omission)",
			args: args{content: `BEGIN:VCALENDAR
VERSION:2.0
X-PUBLISHED-TTL:PT4H
X-WR-CALNAME:test
...
END:VCALENDAR`,
			...
			wantProps: ParsedVTODOProperties{Title: true, Deadline: true},
		},
		{
			name: "with unparseable deadline (must not wipe deadline)",
			args: args{content: `BEGIN:VCALENDAR
VERSION:2.0
X-PUBLISHED-TTL:PT4H
X-WR-CALNAME:test
...
END:VCALENDAR`,
		...
		{
			name: "Format Task with estimated duration as CalDAV",
			args: args{
				list: &models.ProjectWithTasksAndBuckets{
					Project: models.Project{
						Title: "List title",
