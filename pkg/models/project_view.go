...
type ProjectView struct {
	...
	// The kind of this view. Can be `list`, `gantt`, `table`, `kanban` or `calendar`.
...
func (pv *ProjectView) Update(s *xorm.Session, a web.Auth) (err error) {
	return updateProjectView(s, pv, a, true)
}

func updateProjectView(s *xorm.Session, pv *ProjectView, a web.Auth, validateBucketConfiguration bool) (err error) {
	...
	// Checked on the resolved ids, not the request: an id the client merely echoed
	// back can resolve to zero, which is no longer a default==done configuration.
	if validateBucketConfiguration {
		if err = pv.checkBucketConfiguration(); err != nil {
			return
		}
	}
