...
func TestTaskPositionCanUpdateValidatesView(t *testing.T) {
	...
	t.Run("view of a foreign project is denied and writes no row", func(t *testing.T) {
		...
		// Fork rule (#88): every denial is the same 404, so the view check is no existence oracle.
		require.True(t, IsErrProjectViewDoesNotExist(err), "want ErrProjectViewDoesNotExist, got %v", err)
