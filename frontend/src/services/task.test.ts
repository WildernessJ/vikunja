...
describe('TaskService.bulkCreate', () => {
	...
	it('sends the fork field values, not just the keys', async () => {
		// Dates as ISO strings: assignData's case-conversion walk mangles raw
		// Date instances, so the stores always hand the constructor strings.
		await new TaskService().bulkCreate([new TaskModel({
			title: 'first',
			projectId: 42,
			deadline: '2026-09-01T12:00:00.000Z' as unknown as Date,
		...
		const payloadTask = post.mock.calls[0][1].tasks[0]
		expect(payloadTask.deadline).toBe('2026-09-01T12:00:00.000Z')
		expect(payloadTask.estimated_duration).toBe(5400)
		expect(payloadTask.repeat_rrule).toBe('FREQ=WEEKLY;BYDAY=MO')
		expect(payloadTask.repeat_from_completion).toBe(true)
	})
